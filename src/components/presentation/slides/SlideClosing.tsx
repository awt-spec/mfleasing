import { motion } from "framer-motion";
import { CheckCircle, Star, ArrowRight, Phone, Mail, Globe } from "lucide-react";
import { SubZoomContainer, StaggerContainer, StaggerItem } from "../SubZoomContainer";

export const SlideClosing = () => {
  return (
    <div className="w-full max-w-5xl mx-auto text-center">
      <SubZoomContainer delay={0.1} direction="zoom">
        <motion.div
          className="inline-flex items-center justify-center w-24 h-24 rounded-2xl bg-primary mb-6 shadow-lg"
          initial={{ scale: 0, rotate: -180 }}
          animate={{ scale: 1, rotate: 0 }}
          transition={{ delay: 0.2, type: "spring", stiffness: 200 }}
        >
          <span className="text-5xl font-bold text-primary-foreground">S</span>
        </motion.div>
      </SubZoomContainer>

      <SubZoomContainer delay={0.4} direction="bottom">
        <h2 className="text-4xl md:text-6xl font-extrabold text-foreground mb-4">
          ¡Gracias!
        </h2>
      </SubZoomContainer>

      <SubZoomContainer delay={0.6} direction="bottom">
        <p className="text-xl text-muted-foreground max-w-2xl mx-auto mb-8">
          <span className="text-primary font-semibold">Sysde</span> - Potenciando los servicios financieros en el mundo digital
        </p>
      </SubZoomContainer>

      {/* Key benefits */}
      <SubZoomContainer delay={0.8} direction="zoom" className="mb-10">
        <StaggerContainer className="flex flex-wrap justify-center gap-4" staggerDelay={0.1} initialDelay={0.9}>
          {[
            "Automatización completa",
            "Gestión integral",
            "Reportes en tiempo real",
            "Alta conectividad",
          ].map((benefit) => (
            <StaggerItem key={benefit}>
              <motion.div
                className="flex items-center gap-2 px-4 py-2 bg-primary/10 rounded-full"
                whileHover={{ scale: 1.05 }}
              >
                <CheckCircle className="w-4 h-4 text-primary" />
                <span className="text-sm font-medium">{benefit}</span>
              </motion.div>
            </StaggerItem>
          ))}
        </StaggerContainer>
      </SubZoomContainer>

      {/* Clients */}
      <SubZoomContainer delay={1.2} direction="top">
        <p className="text-sm text-muted-foreground mb-4">Líderes que confían en nosotros</p>
        <div className="flex items-center justify-center gap-8 mb-10">
          {["BANORTE", "HABITAT", "Credix"].map((client, index) => (
            <motion.div
              key={client}
              className="flex items-center gap-1"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 1.4 + index * 0.1 }}
            >
              <Star className="w-4 h-4 text-amber-500" />
              <span className="text-lg font-bold text-foreground">{client}</span>
            </motion.div>
          ))}
        </div>
      </SubZoomContainer>

      {/* Contact */}
      <SubZoomContainer delay={1.6} direction="bottom">
        <motion.div
          className="p-6 rounded-2xl bg-card border border-border shadow-lg inline-block"
          whileHover={{ scale: 1.02 }}
        >
          <h3 className="font-bold text-lg mb-4">Contáctanos</h3>
          <div className="flex flex-wrap justify-center gap-6">
            <motion.div
              className="flex items-center gap-2 text-muted-foreground"
              whileHover={{ color: "hsl(358, 78%, 45%)" }}
            >
              <Globe className="w-5 h-5" />
              <span className="text-sm">www.sysde.com</span>
            </motion.div>
            <motion.div
              className="flex items-center gap-2 text-muted-foreground"
              whileHover={{ color: "hsl(358, 78%, 45%)" }}
            >
              <Mail className="w-5 h-5" />
              <span className="text-sm">info@sysde.com</span>
            </motion.div>
            <motion.div
              className="flex items-center gap-2 text-muted-foreground"
              whileHover={{ color: "hsl(358, 78%, 45%)" }}
            >
              <Phone className="w-5 h-5" />
              <span className="text-sm">Contactar</span>
            </motion.div>
          </div>
        </motion.div>
      </SubZoomContainer>

      {/* CTA */}
      <SubZoomContainer delay={1.9} direction="zoom" className="mt-8">
        <motion.button
          className="px-8 py-4 bg-primary text-primary-foreground rounded-xl font-bold text-lg shadow-lg inline-flex items-center gap-2"
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.98 }}
        >
          Solicitar Demo
          <ArrowRight className="w-5 h-5" />
        </motion.button>
      </SubZoomContainer>
    </div>
  );
};
